// deleting the whole tree by using a function.
#include <bits/stdc++.h>
...
// print in post-order
// first print node's leaf children, then print the node itself.
...
// then the root has to be deleted.
// so we will follow post-order traversal.

void deleteTree(TreeNode<int> *root)
{
    // traversing root node's children
    for (int i = 0; i < root->children.size(); i++)
    {
...
int main()
{
    ...
    // deleteTree(root);
    // if we do: delete tree
    // only root means root data and vector will be deleted.
    // but root's children won't be deleted.
    // so we can use this deleteTree() function
    // or update the destructo of tree
    // we have done alreday. check out the tree class
    // now "delete root" is enough to delete the whole root.
