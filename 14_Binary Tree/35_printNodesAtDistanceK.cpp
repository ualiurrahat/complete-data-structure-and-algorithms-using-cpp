...
// helper function to find lca node of the binary tree
BinaryTreeNode<int> *findLcaRoot(BinaryTreeNode<int> *root, int x, int y)
{
    // base case
    ...
    BinaryTreeNode<int> *left = findLcaRoot(root->left, x, y);
    BinaryTreeNode<int> *right = findLcaRoot(root->right, x, y);
    ...
    {
        // left and right are not null. means we already got the result
        return root;
    }
    // S:O(N), recursive stack space
    // T:O(N), at max when given tree is skewed.
}
// function to find the data of the lca node.
int lowestCommonAncestor(BinaryTreeNode<int> *root, int x, int y)
{
    BinaryTreeNode<int> *lcaNode = findLcaRoot(root, x, y);
    return lcaNode->data;
}
// function to find maximum width of a binary tree
int widthOfBinaryTree(BinaryTreeNode<int> *root)
{
    ...
    int ans = 0; // max width
    // queue to store nodes in the level order traversal
    queue<pair<BinaryTreeNode<int> *, int>> q;
    // insert root node in the queue
    q.push({root, 0});
    // to make index of node starting from zero
    int minValue = q.front().second;
    int first, last;
    // level order traversal until queue is not empty
    while (!q.empty())
    {
        int size = q.size();
        for (int i = 0; i < size; i++)
        {
            int currIndex = q.front().second - minValue;
            // extract the front node from queue
            ...
            if (node->left)
            {
                q.push({node->left, (long long)currIndex * 2 + 1});
            }
            if (node->right)
            {
                q.push({node->right, (long long)currIndex * 2 + 2});
            }
        }
...
// function to apply children sum property in a binary tree
...
// This function marks parents of each node in the binary tree.
// It stores parent-child relationships in the parentTrack map.
void markParents(BinaryTreeNode<int> *root, unordered_map<BinaryTreeNode<int> *, BinaryTreeNode<int> *> parentTrack, BinaryTreeNode<int> *target)
{
    queue<BinaryTreeNode<int> *> q;
    q.push(root);
    while (!q.empty())
    {
        ...
        if (current->left)
        {
            parentTrack[current->left] = current; // Store parent of left child
            q.push(current->left);
        }
        if (current->right)
        {
            parentTrack[current->right] = current; // Store parent of right child
...
// This function prints nodes at distance K from the target node in the binary tree.
vector<int> printNodesAtDistanceK(BinaryTreeNode<int> *root, BinaryTreeNode<int> *target, int K)
{
    ...
    visited[target] = true; // Mark target node as visited
    int currLevel = 0;      // Current level in BFS traversal
    while (!q.empty())
    {
        int size = q.size();
        if (currLevel++ == K)
            break; // Stop traversal if distance K is reached
    ...
    // Store nodes at distance K in the result vector
    vector<int> result;
    while (!q.empty())
    {
...
int main()
{
    ...
    int k = 2;
    BinaryTreeNode<int> *target = root->left->right;
    cout << target->data << endl;
    vector<int> ans = printNodesAtDistanceK(root, target, k);
    cout << "Nodes at distance " << k << " from target node are: " << endl;
