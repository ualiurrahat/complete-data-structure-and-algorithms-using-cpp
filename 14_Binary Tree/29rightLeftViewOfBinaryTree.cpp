/*The right view of a Binary Tree is a set of nodes visible when
 the tree is viewed from the Right side and
  the nodes are printed from top to bottom order.
...
int main()
{
    ...
    vector<int> l = printLeftView(root);
    vector<int> r = printRightView(root);
