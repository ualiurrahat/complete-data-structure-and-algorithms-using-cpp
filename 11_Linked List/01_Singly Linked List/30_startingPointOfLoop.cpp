...
int main()
{
    ...
    // now creating link to make the loop with node having data 3 and 8
    Node *startingNode = head->next->next;
    tail->next = startingNode;

    // Node *firstNode = firstNodeOfLoopBrute(head);
    Node *firstNode = firstNodeOfLoopBetter(head);
    cout << "Starting loop's node address: " << firstNode << endl;
    cout << "Starting loop's node data: " << firstNode->data << endl;
    return 0;
}
